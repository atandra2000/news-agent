# newsagent — Architecture & Migration Design

> Pivot plan: transform AIRIP (multi-user SaaS) into **newsagent**, a fully
...
REST APIs, auth, and per-user rankings**. newsagent needs none of that. The infra
...
|---|---|---|
| `llm/router.py` | Best-in-class role-routed router: retry/backoff+jitter, fallback chain, token accounting, OTel spans. **Exactly** what newsagent needs. | Hardcoded Ollama `/api/generate` → extract a `Provider` plugin. |
...
| Config | **Simplify** | `NEWSAGENT_` prefix; drop pg/redis/email/otel-server. |

---

## 3. Target Architecture — newsagent

```
                         ┌─────────────────────────────────────────────┐
                         │            newsagent run  (one-shot / day)       │
                         └───────────────────────┬─────────────────────┘
                                                 │
   ┌─────────────── COLLECT ─────────────────────┼────────────────────────┐
   │  collectors/* (plugin)                       │                         │
...
**Recommendation: clean new package `src/newsagent/`, not in-place surgery.** The
...
**Steps:**
1. Keep `src/airip/` intact as a reference during the build. Do not delete until newsagent produces its first real report.
...
5. Once `newsagent run` produces a report end-to-end, delete `src/airip/`, `docker-compose.yml`, `Dockerfile*`, the 8 API modules, and the tenant migrations. Archive the old `IMPLEMENTATION_PLAN.md`/`PHASE1_DESIGN.md` to `_archive/`.
...
```
newsagent/
  pyproject.toml                 # name="newsagent", NEWSAGENT_ env prefix
  src/newsagent/
    __init__.py
    __main__.py                  # entry: `python -m newsagent run`
    config.py                    # simplified Pydantic Settings (NEWSAGENT_*)
  ...
  storage/
    newsagent.db                    # SQLite corpus + analyses + trends
...
`newsagent run [--date YYYY-MM-DD] [--dry-run] [--stage collect|analyze|…]` runs
...
ordered so newsagent is *partially runnable early* and grows in capability.

| # | Phase | Sessions | Deliverable / Exit criterion |
|---|---|---|---|
| **0** | **Scaffold** | 1 | `src/newsagent/`, simplified `config.py` (`NEWSAGENT_`), `logging.py`/`errors.py` copied, ruff/mypy/pytest green, empty `cli.py`. |
...
| **8** | **Orchestrator + CLI + Scheduler** | 2 | `pipeline/run.py` (stages, resume, backfill); `newsagent run/backfill/status/sources`; launchd + systemd + cron files. **First end-to-end autonomous report.** |
...
**First runnable milestone:** end of **Phase 8** — `newsagent run` produces
...
- **One-shot CLI over Celery** — a crashed run is just a missed day, not a dead worker; OS scheduler is more reliable than a self-hosted broker. Dev loop also gets `newsagent serve --loop`.
...
| `newsagent serve --loop` | Dev convenience, not a product feature. Drop unless wanted. |
...
| **8** | **Quality loop**: diff newsagent vs Perplexity on the same day; tune prompts/selection | The actual optimization engine |
...
Periodically run both newsagent and "Perplexity: today's AI news" on the same day's
inputs and tune until newsagent wins on coverage, depth, and verification — not on
architecture.
...
| 7 | Long-term memory | **Accept (reframed)** | = the persistent stores newsagent already writes (SQLite + Qdrant + trend + KG). Not a new subsystem. |
...
"Long-term memory" = the persistent stores newsagent already writes, queried over
time: (a) SQLite corpus + analyses, (b) Qdrant vectors, (c) trend snapshots,
(d) the §12.4 KG. **Do not build a separate memory service.** newsagent becomes more
