"""YouTube collector (Data API v3). Env-gated: requires NEWSAGENT_YOUTUBE_API_KEY.
Skips gracefully when unset (NEWSAGENT_DESIGN §11.2).
"""
...
class YouTubeCollector(CollectorAdapter):
    ...
    async def collect(self, *, since: datetime, limit: int = 50) -> list[RawItem]:
        api_key = os.environ.get("NEWSAGENT_YOUTUBE_API_KEY")
        if not api_key:
