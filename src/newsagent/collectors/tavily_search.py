...
NEWSAGENT_SEARCH_TAVILY_API_KEY is unset. Key sourced from SearchConfig.
"""
...
from newsagent.config import SearchConfig
...
class TavilySearchCollector(CollectorAdapter):
    ...
    async def collect(self, *, since, limit: int = 50) -> list[RawItem]:
        if not self.api_key:
            log.info("tavily.skipped_no_key", hint="set NEWSAGENT_SEARCH_TAVILY_API_KEY to enable")
            return []
