"""Simplified Pydantic settings. ``NEWSAGENT_`` env prefix. No pg/redis/email/otel."""
...
class CollectorConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="NEWSAGENT_COLLECTOR_", env_file=".env", extra="ignore")
    ...
    enabled: list[str] = Field(
        default_factory=lambda: [
            ...
            "rss",
            "github_trending",
            "github_topic_search",  # GitHub search API by topic; uses NEWSAGENT_GITHUB_TOKEN
            "github_releases",
            "huggingface",
            "blog",
            "hacker_news",
            ...
            "x_twitter",  # requires NEWSAGENT_X_BEARER_TOKEN; no-ops gracefully if unset
            # "tavily" — opt-in: gated on NEWSAGENT_COLLECTOR_TAVILY_API_KEY being set
...
class LLMConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="NEWSAGENT_LLM_", env_file=".env", extra="ignore")
...
class EmbedderConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="NEWSAGENT_EMBED_", env_file=".env", extra="ignore")
...
class PipelineConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="NEWSAGENT_PIPELINE_", env_file=".env", extra="ignore")
...
class StorageConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="NEWSAGENT_STORAGE_", env_file=".env", extra="ignore")

    dir: Path = DEFAULT_STORAGE_DIR
    # SQLite filename inside storage dir.
    sqlite_file: str = "newsagent.db"
    ...
    qdrant_collection: str = "newsagent"
    # Optional Obsidian vault dir to mirror reports into.
    obsidian_vault: str | None = None


class SearchConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="NEWSAGENT_SEARCH_", env_file=".env", extra="ignore")

    # "tavily" needs NEWSAGENT_SEARCH_TAVILY_API_KEY; "none" grounds only in the LLM's
...
class RAGConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="NEWSAGENT_RAG_", env_file=".env", extra="ignore")
...
class ReportConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="NEWSAGENT_REPORT_", env_file=".env", extra="ignore")
...
def load_settings() -> NewsAgentSettings:
