...
async def judge(ctx: RunContext, newsagent_text: str) -> dict:
    """Self-assessment of newsagent' own report (no external baseline)."""
    prompt = (
    ...
    # Heuristic fallback.
    scores = _heuristic_scores(newsagent_text)
    return {
        "scores": scores,
...
async def run_quality(ctx: RunContext, run_date: datetime, settings) -> QualityReport:
    newsagent_path = settings.reports_dir / f"{run_date.strftime('%Y-%m-%d')}.md"
    newsagent_text = newsagent_path.read_text(encoding="utf-8", errors="ignore") if newsagent_path.exists() else None

    if not newsagent_text:
        rep = QualityReport(run_date=run_date.strftime("%Y-%m-%d"), notes=["No report found for date."])
        rep.path = str(newsagent_path)
        return rep

    verdict = await judge(ctx, newsagent_text)
    scores = verdict.get("scores", {})
    total = sum(scores.values()) / max(len(scores), 1)

    rep = QualityReport(
        run_date=run_date.strftime("%Y-%m-%d"),
        newsagent_score=round(total, 2),
    ...
    log.info("quality.done", score=rep.newsagent_score, notes=len(rep.notes))
    return rep


def _render_quality(rep: QualityReport) -> str:
    lines = [f"# newsagent Quality Report — {rep.run_date}", ""]
    lines.append(f"- **Self-score:** {rep.newsagent_score}/5")
    lines.append("")
