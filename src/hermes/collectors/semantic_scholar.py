"""Semantic Scholar collector. No auth by default; set HERMES_SS_API_KEY for
...
from hermes.collectors.http import aget_json
...
class SemanticScholarCollector(CollectorAdapter):
    ...
    async def collect(self, *, since: datetime, limit: int = 50) -> list[RawItem]:
        ...
        api_key = os.environ.get("HERMES_SS_API_KEY")
        headers = {"User-Agent": "hermes/0.1"}
        if api_key:
